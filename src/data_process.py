...
RATING_FILE_NAME = dict({'book': 'BX-Book-Ratings.csv'})

SEP = dict({'book': ';'})

THRESHOLD = dict({'book': 0})

parser = argparse.ArgumentParser()
parser.add_argument('--dataset', type=str, default='book', help='which dataset to preprocess')
...
def convert_kg():
    ...
    for file in files:
        for line in file:
            ...
            if '{}={}={}'.format(head, relation, tail) not in writed_triples:
                writer.write('%d\t%d\t%d\n' % (head, relation, tail))
                writed_triples.add('{}={}={}'.format(head, relation, tail))
